...
impl LlmService {
    ...
    fn health(&self) -> Vec<LlmProviderHealth> {
        ...
        vec![self.template.health(), self.local.health(), self.api.health()]
    }
...
pub fn get_llm_provider_health(
    ...
    let service = state
        .service
        .lock()
        .map_err(|_| CommandError::from(LlmError::State("llm service lock was poisoned".to_string())))?;
    Ok(service.health())
}
...
fn parse_http_endpoint(endpoint: &str) -> Result<HttpEndpoint, LlmError> {
    let endpoint = endpoint
        .strip_prefix("http://")
        .ok_or_else(|| LlmError::InvalidEndpoint("only http:// endpoints are supported".to_string()))?;
    let authority = endpoint.split('/').next().unwrap_or(endpoint);
    let (host, port) = authority
        .rsplit_once(':')
        .ok_or_else(|| LlmError::InvalidEndpoint("endpoint must include host and port".to_string()))?;
    let port = port
        .parse::<u16>()
...
fn post_json(endpoint: &HttpEndpoint, path: &str, body: &str) -> Result<String, LlmError> {
    ...
    if !headers.starts_with("HTTP/1.1 200") && !headers.starts_with("HTTP/1.0 200") {
        return Err(LlmError::Protocol("llama server returned non-200 response".to_string()));
    }
